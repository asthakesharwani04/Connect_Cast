...
export async function signup(req, res) {
  ...
  try {
    ...
    if (existingUser) {
        return res.status(400).json({message: "Email already exists, please use a diifferent one"});
    }

    const idx = Math.floor(Math.random() * 100) + 1; //generate a random num between 1-100 for profile picture
...
// The function updates a user's profile with onboarding information and marks them as "onboarded".
